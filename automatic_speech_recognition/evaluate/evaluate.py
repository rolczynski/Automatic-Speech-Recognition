...
def calculate_error_rates(pipeline: pipeline.Pipeline,
    ...
    for data, transcripts in generator:
        if prepared_features:
            predictions = pipeline.predict(features=data)
        else:
            predictions = pipeline.predict(batch_audio=data)
        batch_metrics = get_metrics(sources=predictions, destinations=transcripts)
...
def get_metrics(sources: List[str], destinations: List[str]) -> Iterable[Metric]:
    ...
    for source, destination in zip(sources, destinations):
        wer_distance, *_ = distance.edit_distance(source.split(), destination.split())
        wer = wer_distance / len(destination.split())
        cer_distance, *_ = distance.edit_distance(list(source), list(destination))
